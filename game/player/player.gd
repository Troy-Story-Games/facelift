...
func _on_mask_detection_area_body_entered(body: Node3D) -> void:
    if wearing_mask:
        # If you try to put another mask on - put the other one back first
        #take_off_mask(wearing_mask.hand_scene_name)
        #var game: Game = get_tree().current_scene as Game
        #var world: World = game.current_scene
        #wearing_mask.reparent(world)
        #match wearing_mask.hand_scene_name:
        #    "spectral_hand":
        #        wearing_mask.global_transform = world.spectral_mask_spawn
        #    "hammer_hand":
        #        wearing_mask.global_transform = world.hammer_mask_spawn
        #wearing_mask = null
        return
